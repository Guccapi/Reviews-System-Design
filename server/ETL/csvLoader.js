...
const seedReview = () => {
  ...
  fs.createReadStream(path.join(__dirname, '../CSVS/reviews.csv'))
    ...
    .on('end', () => {
      ...
      const metaInsertion1 = pgp.helpers.insert(reducedArray.slice(0, reducedArray.length / 4), metaDataColumns);
      const metaInsertion2 = pgp.helpers.insert(reducedArray.slice(reducedArray.length / 4, reducedArray.length / 2), metaDataColumns);
      const metaInsertion3 = pgp.helpers.insert(reducedArray.slice(reducedArray.length / 2, (reducedArray.length / 4) * 3), metaDataColumns);
      const metaInsertion4 = pgp.helpers.insert(reducedArray.slice(-(reducedArray.length / 4)), metaDataColumns);
      const t3 = performance.now();
      queryReview(metaInsertion1, metaInsertion2, metaInsertion3, metaInsertion4);
